function qsync () {
	...
	this.serial = function (array, tasks, callback) {
		...
		clone.select(clone.middleware.serial);
		clone.process(arguments);
	...
	this.parallel = function (array, tasks, callback) {
		...
		clone.select(clone.middleware.parallel);
		clone.process(arguments);
	...
	function refine (input) {
		...
		if (!parameters.length) {
			...
			if (isTasks(last)) {
				...
				console.error("Last argument has to be either tasks or callback");
	...
	function add (data, label, callback) {
		...
		self.engine.add(function (result, callback) {
	...
	function run () {
		...
		self.engine.run([null], function (error, results) {
			if (input.multiplicity == "multiple") {
				input.callback(error, results);
			} else {
				input.callback(error, results[0]);
			}
		});
	}
